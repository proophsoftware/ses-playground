...
import {UpdateCar, updateCar} from "@app/fleet-management/model/car/commands/update-car";
import dispatch from "@app/fleet-management/command-bus/dispatch-update-car";

type Params = BodyParams<UpdateCar>;
...
try {
  ...
  const success = await dispatch(updateCar(event.body, {userId: context.user?.id}));
